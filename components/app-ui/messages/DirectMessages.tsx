...
function DirectMessages({ 
  ...
  const {setCurrentContact, contacts, currentContact} = useContext(ContactContext)

  return (
    <div className="mt-4 w-full   shrink-0 h-full  ">
      <div className="text-xs font-semibold text-slate-400 uppercase mb-3">
        Messages
      </div>
      {contacts.loading && <Spinner color="green"/>}
      {contacts.error && 'Error'}

      <ul className="mb-6 w-full space-y-1 customscroll">
        {contacts.data && contacts.data.map((elem:any, index:number) => {
          return (
            <li key={`contacts_${elem._id}`} className={`${elem._id === currentContact._id ? 'bg-blue-200 hover:bg-blue-300' : 'hover:bg-blue-100'}  rounded-md w-full`}>
              <button
                className="flex items-center gap-4 w-full p-2 rounded-md"
                onClick={() => {
                  setMsgSidebarOpen(false)
                  setCurrentContact(elem)
                }}
              >
                <div className="w-10 h-10 rounded-full bg-slate-500 shrink-0 flex items-center justify-center">
                  <span className="uppercase text-white">{elem.full_name[0]}</span>
                </div>
