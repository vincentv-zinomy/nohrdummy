...
interface AddNewMessageTemplateModalProps {
    show: boolean;
    setShow: (show: boolean) => void;
}
function AddNewMessageTemplateModal({
    show,
    setShow,
}: AddNewMessageTemplateModalProps) {
    ...
    const [selectedWabaId, setSelectedWabaId] = useState<string>(""); // New error state
    const [messageTemplateData, setMyMessageTemplateData] = useState<MessageTemplateResponseType>({
    ...
    const getAllAvailableWhatsAppIds = async () => {

        try {
            const getData = await axiosAPIWithAuth.get("/message-template/available-waba-numbers");
            const resData = await getData.data;

            setWabaData(resData);
            if (resData.length > 0) {
                setSelectedWabaId(resData[0].waba_id);
            }
    ...
    const addMessageTemplate = async () => {
        ...
        try {
            if (messageTemplateData.template_type === "WHATSAPP") {
                await axiosAPIWithAuth.post("/message-template/create-template-whatsapp", {
                    ...messageTemplateData,
                    waba_id: selectedWabaId
                });

                toast.addToast("success", "New Message Template added successfully");
                // Reset form fields

                // Close the modal
                setShow(false);
            }
            if (messageTemplateData.template_type === "SMS") {
                await axiosAPIWithAuth.post("/message-template/create-template-sms", {
                    ...messageTemplateData
                });
                                                ...
                                                <div className="sm:col-span-6 sm:w-full">
                                                    ...
                                                    <div className="mt-1">
                                                        ...
                                                        >
                                                            ...
                                                            <option value="WHATSAPP" disabled={selectedWabaId.length === 0}>Whatsapp</option>
...

                                                {
                                                    messageTemplateData.template_type === "WHATSAPP" && (
                                                        <>
                                                            ...
                                                            >
                                                                {
                                                                    wabaData && wabaData.map((elem, el_in) => {
                                                                        return (

                                                                            <option value={elem.waba_id}>{elem.number}</option>
                                                            ...
                                                            </div>
                                                            {
                                                                (messageTemplateData.template_data.sample_values).map((elem, el_index) => {
                                                                    return (
                                                                        <div className="sm:col-span-6 sm:w-full mt-2" key={`${el_index}_waba_data_sample_values`}>
                                                                            <label className="block text-sm font-medium text-gray-700">
                                                                                {` Example Value for {{${el_index + 1}}}`}
                                                                            </label>
                                                                            <div className="mt-1">
                                                                                <input
                                                                                    value={elem}
